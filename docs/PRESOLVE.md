...
The CUDA backend uploads immutable `A`, direct-cone metadata, and input affine
cone data `(G, h)` into one model workspace and keeps them resident across
presolve passes. Bounds, row sides, and removal flags are synchronized when
needed. If affine cone-coordinate aggregation changes direct-cone ownership,
the workspace is invalidated and rebuilt before its next use.
...
The same structural setting enables a two-phase `A` compaction path when at
least five percent of the rows or columns have been removed and no recorded
column substitution must be expanded. The device counts and scatters retained
entries.
Fixed-variable row shifts and empty-row feasibility are recomputed on the CPU
with the regular extended-precision checks before the compact matrix is
committed. `Q`, `R`, and transformed or generated affine matrices retain their
CPU compaction paths.
...
constraints on cone variables.

For input affine cones, coordinate activities for `Gx + h`, SOC and rotated-SOC
envelopes, and coordinate-to-variable bound propagation run on the device.
Exponential, power, and PSD envelopes use the device activities but retain CPU
nonlinear or higher-order verification. Affine cones generated by coordinate
aggregation currently use the CPU path because their rows are transformation
records rather than immutable input CSR. Facial reductions, transformation
logging, and final candidate commits remain on the CPU.
...
workspace setup, parallel-row, cone-activity, direct- and affine-cone,
compaction, transfer, kernel, round, and fallback statistics. Both GPU settings
are disabled by default because small models may not amortize context setup and
kernel launches.
...
With `structural_reductions_gpu = 1`, PreFOS also builds a deterministic CSC
view of active `A` entries on the device. Singleton-column selection is compacted
there before CPU verification, while parallel columns are hashed and sorted on
the GPU and then checked exactly before any substitution is committed. The
second singleton and parallel-column passes reuse one column workspace.
